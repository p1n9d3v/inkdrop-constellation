...
function clampLabelX(element, x, width, obj) {
    if (obj.labelWidth === undefined || obj.labelWidth === 0) {
        obj.labelWidth = element.offsetWidth || 0;
    }
    const halfWidth = Math.min(obj.labelWidth / 2, Math.max(0, width / 2 - LABEL_MARGIN));
...
const ConstellationGraph = ({ data, hideOrphans, showBooks, isDragging = false, onAfterOpen, }) => {
    ...
    const triggerResizeRef = React.useRef(() => { });
    ...
    React.useEffect(() => {
        ...
        function updateBooks(width, height, formed) {
            ...
            for (const book of renderBooks) {
                ...
                const x = clampLabelX(book.label, (tmpScreen.x * 0.5 + 0.5) * width, width, book);
        ...
        function updateNodeLabels(width, height, formed) {
            ...
            for (let i = 0; i < renderNodes.length; i++) {
                ...
                const x = clampLabelX(node.labelEl, (tmpScreen.x * 0.5 + 0.5) * width, width, node);
                const y = (-tmpScreen.y * 0.5 + 0.5) * height;
                const targetOpacity = String(i === hoveredIndex || endpoint ? 1 : node.hub ? 0.82 : 0.52);
