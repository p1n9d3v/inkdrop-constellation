...
export async function loadGraph(): Promise<GraphData> {
  ...
  const docs: any[] = notesRes.docs;
  const tagList: any[] = Array.isArray(tagsAll) ? tagsAll : tagsAll?.docs ?? [];
  const bookList: any[] = Array.isArray(booksAll)
    ? booksAll
    : booksAll?.docs ?? [];

  const tagColorById = new Map<string, string>();
  ...
  const nodes: GraphNode[] = docs.map((n) => {
    const title = (n.title || "").trim() || "(untitled)";
    const firstTagId: string | undefined = n.tags?.[0];
  ...
  for (const note of docs) {
    const bid: string | undefined = note.bookId;
    if (!bid) continue;
    if (!memberMap.has(bid)) memberMap.set(bid, []);
    memberMap.get(bid)!.push(note._id);
  }

  const books: GraphBook[] = bookList
